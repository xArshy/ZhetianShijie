...
function tbMagic:FiveBase(rate)
    ...
    if self.bind.LuaHelper:IsLearnedEsoteric("TTMG_2_1") then
    ...
    if
        (self.bind.PropertyMgr.Perception >= 10 and self.bind.PropertyMgr.Physique >= 10 and
            self.bind.PropertyMgr.Charisma >= 10 and
            self.bind.PropertyMgr.Luck >= 10 and
            self.bind.PropertyMgr.Intelligence >= 10) or
            (target.PropertyMgr.Perception <= 0 and target.PropertyMgr.Physique <= 0 and
                target.PropertyMgr.Charisma <= 0 and
                target.PropertyMgr.Luck <= 0 and
                target.PropertyMgr.Intelligence <= 0)
     then
        return 1
    end
end
...
function tbMagic:FiveSudden()
    local target = ThingMgr:FindThingByID(self.targetId)
    self.bind.PropertyMgr.BaseData:AddAddion(
        CS.XiaWorld.g_emNpcBasePropertyType.Perception,
        target.PropertyMgr.Perception
    )
    target.PropertyMgr.BaseData:AddAddion(
        CS.XiaWorld.g_emNpcBasePropertyType.Perception,
        -target.PropertyMgr.Perception
    )
    ...
    self.bind.PropertyMgr.BaseData:AddAddion(
        ...
        target.PropertyMgr.Intelligence
    )
    target.PropertyMgr.BaseData:AddAddion(
        CS.XiaWorld.g_emNpcBasePropertyType.Intelligence,
        -target.PropertyMgr.Intelligence
