...
function GameUlt.ZhaoLanwsst(region)
	...
	local desc = nil
	if (pmname == nil) then
		desc = string.format("Nowadays%s, %s started to recruit disciples，%s。Number of agents，Total%s people come。", timesec, region.DisplayName, humandesc, human)
	else
		desc = string.format("Nowadays%s, %s agent%s started to recruit disciples，%s。Number of agents，Total%s people come。", timesec, region.DisplayName, pmname, humandesc, human)
	end

	WorldLua:ShowStoryBox(
		desc, "Solicit", {"Solicit"}, 
		function(index)
