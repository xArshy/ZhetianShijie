...
npc.PropertyMgr:AddFeature("ZTofBody");
npc.LuaHelper:AddTitle("Pursuit of the Eucharist","After the ancient times, I did not know a few people who hunted down the Eucharist",2);
npc.PropertyMgr.Practice:Up2Disciple(CS.XiaWorld.PracticeMgr.Instance:RandomGong(),12);
npc.PropertyMgr.Practice:RandomTree();
npc.PropertyMgr:SetSex(CS.XiaWorld.g_emNpcSex.Male)
npc.BodyColor = 4;
...
if count == 1 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.AtkRateDamageAddp,Desc ="天璇心法：20%几率对敌方法宝造成6倍伤害",fParam1="0.20" ,fParam2="5" ,nParam1="2"};
fabao.Fabao:AddAbilityData(data);
 end
 if count == 0 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.HitCountAddSubColdDown,Desc ="行字无双：每攻击12次减少20s术法cd",nParam1="12",fParam1="20"};
fabao.Fabao:AddAbilityData(data);
 end
...
npc:EquipItem(fabao);
if count == 1 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.AtkRateDamageAddp,Desc ="破灭：50%几率对敌方法宝造成10倍伤害",fParam1="0.50" ,fParam2="9" ,nParam1="2"};
fabao.Fabao:AddAbilityData(data);
 end
 if count == 0 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.AgeAddPowerUp,Desc ="神伤：年龄越大伤害越高",fParam1="2" ,fParam2="0"};
fabao.Fabao:AddAbilityData(data);
 end
  if count == 2 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.HitCountAddSubColdDown,Desc ="君临天下：每攻击3次减少10s术法cd",nParam1="3",fParam1="10"};
fabao.Fabao:AddAbilityData(data);
 end
 if count == 3 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.HitCountCostSkill,Desc ="万念花开：每攻击30次释放一次万化圣诀",nParam1="30",fParam1="TTMG_WHSJ"};
fabao.Fabao:AddAbilityData(data);
 end
 if count == 4 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.HitSameFabaoAndEffect,Desc ="吞天魔功：每攻击10次使对手法宝无效",nParam1="10",nParam2="1"};
fabao.Fabao:AddAbilityData(data);
 end
 if count == 5 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.HitCountAddMirror,Desc ="吞天魔功：每攻击10次分出10道分影，持续10s",nParam1="10",nParam2="10",fParam1="10"};
fabao.Fabao:AddAbilityData(data);
 end
fabao:AddLing(99999999999);
count = count + 1;
...
fabao.Fabao:AddAbilityData(data);
 ...
 if count == 0 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.HitCountAddSubColdDown,Desc ="天璇圣法：每攻击10次减少9s术法cd",nParam1="10",fParam1="9"};
fabao.Fabao:AddAbilityData(data);
 end
item:AddLing(99999999999);
npc:EquipItem(item,CS.XiaWorld.g_emEquipType.None)
...
npc:EquipItem(fabao);
if count == 0 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.HitCountAddSubColdDown,Desc ="虚空经：每攻击21次减少10s术法cd",nParam1="21",fParam1="10"};
fabao.Fabao:AddAbilityData(data);
 end
if count == 1 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.HitCountCostSkill,Desc ="虚空大手印：每攻击30次释放一次虚空大手印",nParam1="30",fParam1="TDJ_DJ_XKDSY"};
fabao.Fabao:AddAbilityData(data);
 end
fabao:AddLing(99999999999);
count = count + 1;
...
npc:EquipItem(fabao);
if count == 0 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.HitCountAddSubColdDown,Desc ="虚空经：每攻击21次减少20s术法cd",nParam1="21",fParam1="20"};
fabao.Fabao:AddAbilityData(data);
 end
if count == 1 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.HitCountCostSkill,Desc ="虚空大手印：每攻击15次释放一次虚空大手印",nParam1="15",fParam1="TDJ_DJ_XKDSY"};
fabao.Fabao:AddAbilityData(data);
 end
 if count == 2 then
local data ={Kind = CS.XiaWorld.Fight.g_emFabaoSpecialAbility.HitCountAddMirror,Desc ="虚断两界：每攻击1次分出4道分影，持续10s",nParam1="1",nParam2="4",fParam1="5"};
fabao.Fabao:AddAbilityData(data);
 end
fabao:AddLing(99999999999);
count = count + 1;
